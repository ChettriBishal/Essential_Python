...
see_friend(friends_last_seen, 'Jen')
...
def increase_age(current_age):
    print(f'Inside function: current age {id(current_age)}')
    current_age = current_age + 1  # new current_age value
    print(f'Inside function: current age {id(current_age)}')
...
increase_age(age)
print(id(age))
print(age)  # the value remains 20

print('\n')

primes = [2, 3, 5]

print(f"Id of primes {id(primes)}")

primes += [7, 11]  # primes = primes.__iadd__([7,11])

print(f"Id of primes after {id(primes)}")  # remains same
