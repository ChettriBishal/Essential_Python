...
The main purpose of this video is to make you aware that these things exist,
in case we have to use them later on (or when you encounter a situation where using one of these would be useful).
...
## Counter
"""
Allows us to count things. Give it a iterable or a mapping (such as a dict) 
and it will turn into a counter of elements.
"""
...
"""
As of Python3.7, I expected `OrderedDict` to be less useful since 
dictionaries themselves will retain the order in which keys are inserted.
"""

## namedtuple
"""
A namedtuple is another object that we can use like a tuple, 
where each of the elements of the tuple has a name. In addition, the tuple itself also has a name.
...
"""
When you’re dealing with data and it doesn’t warrant creating classes for the data elements 
you’re working with, `namedtuple` is a great choice (and very flexible!).
"""
