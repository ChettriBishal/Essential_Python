...
account_2 = create_account('savings', 'Anne')
...
The default parameter for the `create_account` function gets evaluated when the function is defined,
 *not when the function is called*.

Because we’re modifying the parameter inside the function,
 the next time we call it the parameter is still the list that has been appended to.
...
"""
In this option we always must pass in an (empty or otherwise) list to the function call. 
It makes things explicit, but it means we may require a lot of empty lists being passed in there.
"""
